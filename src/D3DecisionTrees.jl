...
function name(node::Node)
    featval = string(isa(node.featval, Real) && !isa(node.featval, Bool) ? round(node.featval,2) : node.featval)
...
function name(leaf::Leaf)
    ...
    majority = string(isa(leaf.majority, Real) && !isa(leaf.majority, Bool) ? round(leaf.majority,2) : leaf.majority)
    "Leaf\n$(majority): $(ratio)"
end
...
function D3DecisionTree(model::Node)
    ...
    modeltreeout = Vector{Vector{Int}}(n)
    namesout = Vector{String}(n)
